...
#create initial data memory        
        ...
        rawDataStrings = re.findall("\w*:\s*#\d*", dataMemString);
        dataMem = {};

        for raw in rawDataStrings:
                variableName = re.findall("\w*(?=:)", raw)[0];
...
#create initial program memory
        programMem = re.findall("\w+:\s*#?\w+|#?\w+|NEXT", programMemString); 
...
#resolve labels in program memory
        for i,val in enumerate(programMem):
                ...
                if re.match("\w+:", val):
                        label = re.findall("\w+(?=:)", val)[0];
                        programMem[i] = re.findall("(?<=:)\s*\w+", val)[0];
